...
unsigned int HuffmanReadUInt(unsigned char *buffer, int *bitOffset, int bits)
{
	...
	if (bits & 7)
	{
		...
		{
			value |= (Huff_getBit(buffer, bitOffset) << i);
		}
	...
	if (bits) 
	{
		...
		{
			Huff_offsetReceive(msgHuff.decompressor.tree, &get, buffer, bitOffset);
			value |= (get << (i + nbits));
		}
	}
